    ...
    public static function message() {
        ...
        if( isset( $ufel_votes['no'] ) ){
            ...
            if( !isset( $ufel_votes['tweet'] ) ) {
                $btn_str .= '<a href="'.admin_url( 'admin-ajax.php' ).'?action=ufel_vote&amp;vote=tweet" class="'.self::$text_domain.'-vote-action '.self::$text_domain.'-vote-button button button-small" data-action="http://twitter.com/share?url=http://bit.ly/2mXDU5t&amp;text='.urlencode( __( self::$tw_title, 'sm' ) ).'">'.__( 'Tweet', 'sm' ).'</a>';
            }
            if( !isset( $ufel_votes['facebook'] ) ) {
                $btn_str .= '<a href="'.admin_url( 'admin-ajax.php' ).'?action=ufel_vote&amp;vote=facebook" class="'.self::$text_domain.'-vote-action '.self::$text_domain.'-vote-button button button-small" data-action="http://facebook.com/sharer?u=http://bit.ly/2mXDU5t&amp;text='.urlencode( __( self::$fb_title, 'sm' ) ).'">'.__( 'Share on facebook', 'sm' ).'</a>';
            }
            if( !isset( $ufel_votes['no'] ) ) {
